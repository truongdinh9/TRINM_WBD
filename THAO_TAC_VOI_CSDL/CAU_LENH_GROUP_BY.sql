...
-- GROUP BY / COUNT
SELECT o.status, COUNT(*) AS 'So luong status' FROM orders o GROUP BY o.status;

-- GROUP By / SUM
SELECT o.status, SUM(od.priceEach * od.quantityOrdered)  AS 'TOTAL'
FROM classicmodels.orders o JOIN classicmodels.orderdetails od ON o.orderNumber = od.orderNumber GROUP BY status DESC ;

--

SELECT od.orderNumber ,SUM(od.quantityOrdered * od.priceEach)  AS 'total'FROM orderdetails od group by od.orderNumber;

-- GROUP BY / HAVING

SELECT YEAR(o.orderDate) AS YEAR, SUM(od.quantityOrdered* od.priceEach) AS 'TOTAL'
FROM classicmodels.orders o JOIN classicmodels.orderdetails od WHERE o.status='Shipped'
